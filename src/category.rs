...
impl Categorizer {
    ...
    pub fn new() -> Self {
        ...
        // Package managers
        for cmd in &[
            "npm", "yarn", "pnpm", "cargo", "pip", "pip3", "gem", "bundle", "apt", "apt-get",
            "brew", "yum", "dnf", "pacman",
        ] {
        ...
        // File operations
        for cmd in &[
            "ls", "cd", "mkdir", "rm", "rmdir", "cp", "mv", "cat", "less", "more", "head", "tail",
            "touch", "find", "grep", "awk", "sed",
        ] {
        ...
        // Network commands
        for cmd in &[
            "curl", "wget", "ping", "ssh", "scp", "rsync", "nc", "netcat", "telnet", "ftp", "sftp",
        ] {
...
mod tests {
    ...
    fn test_network_categorization() {
        ...
        assert_eq!(
            categorizer.categorize("curl https://example.com"),
            "network"
        );
    ...
    fn test_cloud_categorization() {
        ...
        assert_eq!(
            categorizer.categorize("gcloud compute instances list"),
            "cloud"
        );
